...
from bcrypt_file import validate_password, hash_password
from typing import NewType, List, Dict

User = NewType('user', Dict[str, str | int])
...
def save_user(user: User) -> None:
    ...
    file_path: Path = get_file_path()
    users: UserList | List = get_users()

    if [u for u in users if u['username'] == users['username']]:
...
def get_user_by_username(username: str) -> User | str:
    users: UserList | List = get_users()
...
print(get_user_by_username('amy_special'))
